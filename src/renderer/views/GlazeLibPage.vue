<template>
  <div class="glaze-lib-page">
    <div class="list-wrapper">
      <draggable class="list" element="ul" v-model="glazeLib">
        <transition-group>
          <li class="item" v-for="item in glazeLib" :key="item.name" @click="chooseItem(item.name)">
            <div class="item-bg">
              <img :src="item.name | toGlazeBgUrl">
            </div>
...
export default {
  ...
  computed: {
    ...
    glazeLib: {
      get () {
        return this.$store.state.Material.glazeLib
      },
      set (value) {
        this.$store.dispatch('updateGlazeLib', value)
      }
    }
  },
  methods: {
    ...mapActions(['setCurrentGlaze', 'addGlaze', 'editGlaze', 'deleteGlaze']),
    navBack () {
      this.$router.go(-1)
    },
    navToMaterialMenu () {
    ...
    chooseItem (name) {
      console.log('chooseItem', name)
      this.setCurrentGlaze(name)
      this.navBack()
    },
    chooseBgFile () {
      ...
      if (!this.newItem.bgUrl) {
        ...
        if (this.currentAction === 'add') {
          if (indexInLib !== -1) {
            ...
            this.addGlaze(this.newItem)
          }
        } else {
          this.editGlaze(this.newItem)
        }
